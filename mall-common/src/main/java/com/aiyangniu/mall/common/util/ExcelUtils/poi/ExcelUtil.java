package com.aiyangniu.mall.common.util.ExcelUtils.poi;
...
/**
 * Excel导入导出工具类（导出多Sheet页存在问题-674行）
 *
 * @author lzq
 * @date 2023/08/16
 */
...
//        ".xls"  --> "application/vnd.ms-excel"
        ...
        // 标识允许哪个域的请求，"*"表示允许跨域，同时浏览器将不会发送cookies，并且出于服务器安全考虑，一般不会这么干
//        response.setHeader("Access-Control-Allow-Origin", "*");
//        response.setHeader("Access-Control-Allow-Headers", "Authentication");
        response.setCharacterEncoding("utf-8");
        String name = new String(fileName.getBytes(StandardCharsets.UTF_8), "ISO8859_1") + XLSX;
