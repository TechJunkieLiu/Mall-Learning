...
import com.aiyangniu.mall.enter.component.CancelOrderSender;
import com.aiyangniu.mall.enter.mapper.*;
import com.aiyangniu.mall.enter.model.bo.*;
...
public class OmsEnterOrderServiceImpl implements OmsEnterOrderService {
    ...
    private final EnterOrderMapper enterOrderMapper;
    ...
    private final EnterOrderItemMapper enterOrderItemMapper;
    ...
    public Integer cancelTimeOutOrder() {
        ...
        // 查询超时、未支付的订单及订单详情
        List<OmsOrderDetail> timeOutOrders = enterOrderMapper.getTimeOutOrders(orderSetting.getNormalOrderOvertime());
        ...
        enterOrderMapper.updateOrderStatus(ids, 4);
        for (OmsOrderDetail timeOutOrder : timeOutOrders) {
            // 解除订单商品库存锁定
            enterOrderMapper.releaseSkuStockLock(timeOutOrder.getOrderItemList());
            // 修改优惠券使用状态
    ...
    public Map<String, Object> generateOrder(OrderParam orderParam) {
        ...
        // 设置自动收货天数
        List<OmsOrderSetting> orderSettings = omsOrderSettingMapper.selectList(new LambdaQueryWrapper<>());
        ...
        enterOrderItemMapper.insertList(orderItemList);
        // 如使用优惠券更新优惠券使用状态
        if (orderParam.getCouponId() != null) {
    ...
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            PmsSkuStock skuStock = pmsSkuStockMapper.selectBySkuId(cartPromotionItem.getProductSkuId());
            skuStock.setLockStock(skuStock.getLockStock() + cartPromotionItem.getQuantity());
            pmsSkuStockMapper.updatePmsSkuStock(skuStock);
        }
    }
    ...
    public Integer paySuccess(Long orderId, Integer payType) {
        ...
        OmsOrderDetail orderDetail = enterOrderMapper.getDetail(orderId);
        int count = enterOrderMapper.updateSkuStock(orderDetail.getOrderItemList());
        return count;
    }
    ...
    public CommonPage<OmsOrderDetail> list(Integer status, Integer pageNum, Integer pageSize) {
        ...
        LambdaQueryWrapper<OmsOrder> lqw = new LambdaQueryWrapper<OmsOrder>().eq(OmsOrder::getDeleteStatus, 0).eq(OmsOrder::getMemberId, member.getId());
        if(status != null){
            lqw.eq(OmsOrder::getStatus, status);
        }
        lqw.orderByDesc(OmsOrder::getCreateTime);
        List<OmsOrder> omsOrderList = omsOrderMapper.selectList(lqw);
    ...
    public OmsOrderDetail detail(Long orderId) {
        ...
        BeanUtil.copyProperties(omsOrder, omsOrderDetail);
    ...
    public void cancelOrder(Long orderId) {
        ...
        if (cancelOrder != null) {
            ...
            if (!CollectionUtils.isEmpty(orderItemList)) {
                enterOrderMapper.releaseSkuStockLock(orderItemList);
            }
            // 修改优惠券使用状态
